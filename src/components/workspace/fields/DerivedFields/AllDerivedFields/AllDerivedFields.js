...
import BetaFeature from '../../../../../components/common/BetaFeature';

import './index.less';

export default function AllDerivedFields({ workspaceId, url }) {
  ...
  const [filteredDerivedFieldsMetaData, setFilteredDerivedFieldsMetaData] =
    useState([]);

  const [derivedFieldsMetaData, setDerivedFieldsMetaData] = useState([]);
  ...
  useEffect(() => {
    if (!isFetchingFieldsMetaData && !isFetchingFieldBundles && fieldBundleId) {
      ...
      const orderedDerivedFields =
        fieldBundles &&
        fieldBundles
          .find(({ id }) => id === fieldBundleId)
          ?.fieldIds.flatMap(fieldId => {
            const workflowField = derivedFields.find(
              ({ id }) => id === fieldId
            );
            return workflowField ? [workflowField] : [];
          });

      setFilteredDerivedFieldsMetaData(orderedDerivedFields);
      setDerivedFieldsMetaData(orderedDerivedFields);
              ...
              <Col span={4}>
                <Input
                  ...
                  onChange={({ target: { value: query } }) => {
                    setFilteredDerivedFieldsMetaData(
                      derivedFieldsMetaData.filter(({ name }) =>
        ...
        >
          {dataFieldsPresent || isFetchingFieldsMetaData ? (
            <Row gutter={[10, 10]}>
                  ...
                  >
                    ...
                    >
                      <Button
                        ...
                        shape="circle"
                        type="primary"
                        title="Create new Derived Field"
              ...
              {(filteredDerivedFieldsMetaData?.length ||
              !isFetchingFieldsMetaData
                ? filteredDerivedFieldsMetaData
                : [{}, {}, {}, {}, {}]
              ).map(item => (
