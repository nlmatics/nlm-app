...
import useSubscriptionPlans from '../../edgar/hooks/useSubscriptionPlans';
...
export default function SubscriptionOptions({ workspaceId }) {
  const {
  ...
  const plan = getSubscriptionPlanById(workspaceId);
  ...
  const pickPlan = async ({ plan, update_payment_info }) => {
    const data = {
      lookup_key: {
        ...plan,
        return_path: `/repository/${workspaceId}/subscribe`,
        update_payment_info,
      },
    };
    try {
          ...
          <Col span={12}>
            <Card title="Enterprise">
              <Typography.Paragraph>
                With
                <Typography.Link href="https://nlmatics.com/" target="_blank">
                  &nbsp;nlmatics&nbsp;
                </Typography.Link>
              ...
              <Typography.Paragraph>
                <ul>
                  ...
                  <li>
                    AI extracted data points for private repository of credit
                    agreements and more
                  </li>
